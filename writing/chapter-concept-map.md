...
- **Introduces:** `RaftLocalState`, `RaftApplyState`, durable `PeerState`, startup reconstruction, `snapshot_raft_state_key`, uninitialized-peer tombstones, split crash recovery, two-path child creation, `pending_snapshot_regions`, `pending_create_peers`, stale-peer GC, `atomic_snap_regions`, merge recovery by target epoch and peer generation, and consecutive-merge ordering.
- **Core flow:** normal lifecycle work persists a peer record -> startup rebuilds or resumes from that record -> a snapshot uses a KV-engine marker to bridge its Raft-engine write -> split persists parent and child topology atomically -> creation and destruction guards choose one owner when split, initial messages, snapshots, and removal overlap -> a saved `Merging` state and the target's current identity decide whether a source must wait, be removed atomically with a snapshot, or be garbage-collected.
