...
In [RAFTSTORE 402](402.md), we followed the event loop for a single Region peer. We saw how the peer FSM processes events, passes incoming Raft messages to `raft-rs`, and handles the resulting `Ready`.
...
Each follower processes the incoming message through the message path described in [RAFTSTORE 402](402.md). Its own `Ready` tells Raftstore to persist the entry in that follower's Raft Engine before responding to the leader.
...
The single-Region event loop from [RAFTSTORE 402](402.md) still exists. The Raft batch system simply processes many instances of that loop together.
...
We will examine this path in more detail in [RAFTSTORE 502: Linearizable Reads](502.md).
