...
The detailed races between split, snapshot-based peer creation, and peer destruction are part of the peer lifecycle and are covered later in [RAFTSTORE 901: Peer Lifecycle and Crash Recovery](901.md). For now, the core picture is enough: a split is a replicated topology change that creates two independent Raft Regions from one key range, while the data remains on the same stores.
