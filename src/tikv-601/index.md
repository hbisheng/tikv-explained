...
There is a more complicated path for **in-memory pessimistic locks**. A leader transfer may ask for a second acknowledgement to transfer that lock state between peers. [TiKV 803](../tikv-803/index.md) introduces that case. For now, consider the no-lock path.
