...
With this picture, we have described the core process of a single-Region Raftstore: it receives an event, drives the Region's `raft-rs` peer, performs the work returned through `Ready`, and continues. This is the execution loop behind the Raftstore layer introduced in [TiKV 301: TiKV Architecture](301.md).
