...
The `step` API is specifically for Raft messages exchanged between peers. New writes use a separate API called `propose`. Like `step`, `propose` feeds new information into the peer's internally managed Raft state.

## `ready`: Ask Raft What Needs to Be Done

TiKV calls `ready` to retrieve the work that `raft-rs` has produced.

TiKV can pass several messages through `step` before calling `ready`. One `Ready` can therefore collect the work produced by multiple incoming messages.
...
```text
feed one or more events into raft-rs
...
With this picture, we have described the core process of a single-Region Raftstore: it receives events, drives the Region's `raft-rs` peer, performs the work returned through `Ready`, and continues. This is the execution loop behind the Raftstore layer introduced in [TiKV 301: TiKV Architecture](301.md).
