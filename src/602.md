...
The exact persistent records and internal messages that pass work between these components are implementation details. [RAFTSTORE 901: Raft Peer Lifecycle](901.md) introduces them later. The important order here is: first persist the Region metadata; then update the in-memory Raft configuration.
