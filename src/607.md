...
[TXN 404](404/index.md) described the data that a transaction leaves in the `write`, `default`, and `lock` column families. This chapter follows one transaction command: how TiKV checks whether it is allowed to write, then turns it into those column-family changes.
...
The scheduler packages these modifications into a Raftstore write. Raftstore replicates and commits them before they become durable Region state, as described in [RAFTSTORE 501](501.md).
...
This chapter covers the classic 2PC path. [TXN 706](706.md) introduces how Async Commit and 1PC shorten that path.
